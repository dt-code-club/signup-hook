...
def sendwelcome(response: dict):
    ...
    <br><br>
    {f"<span style='color:red'> Unfortunately, we couldn't find you in the school database ({userinfo['firstname']} {userinfo['lastname']} - {userinfo['snum']}) and cannot extend an invite to our Discord server. If you do indeed attend the school, please send us an email at this address (<a href='mailto:dtcodeclub@gmail.com' style='color:#ff5656 !important;'>dtcodeclub@gmail.com</a>) and we'll try our best to fix this problem.</span><br><br>" if not userinfo["attends"] else ""}
